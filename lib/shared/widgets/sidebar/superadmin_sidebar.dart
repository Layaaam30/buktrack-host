...
class SuperAdminSidebar extends StatelessWidget {
  ...
  Widget _buildHeader(ThemeData theme, bool isDark, bool isMobile) {
    return Container(
      height: isExpanded ? AppSizes.headerHeight : null,
      padding: EdgeInsets.symmetric(
        horizontal: AppSizes.sm,
        vertical: isExpanded ? AppSizes.md : AppSizes.xs,
      ),
      decoration: BoxDecoration(
        border: Border(
          bottom: BorderSide(
          ...
          child: Column(
            ...
            children: [
              ...
              SidebarLink(
                ...
                isExpanded: isExpanded,
                isActive: navigationProvider.isActive('/companies'),
                onTap: () {
                  navigationProvider.navigateTo('/companies');
                  if (isMobileOpen) onClose();
                },
              ),
              ...
              SidebarLink(
                ...
                isExpanded: isExpanded,
                isActive: navigationProvider.isActive('/admins'),
                onTap: () {
                  navigationProvider.navigateTo('/admins');
                  if (isMobileOpen) onClose();
                },
              ),
  ...
  Widget _buildUserSection(ThemeData theme, bool isDark, BuildContext context) {
    return Consumer<AuthProvider>(
      builder: (context, authProvider, child) {
        final userName =
            authProvider.currentSuperAdmin?.name ??
            authProvider.currentAdmin?.name ??
            'SuperAdmin';
        final userEmail =
            authProvider.currentSuperAdmin?.email ??
            authProvider.currentAdmin?.email ??
            'superadmin@buktrack.com';
  ...
  Widget _buildExpandedUserSection(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _buildUserAvatar(isDark, firstLetter, true),
            const SizedBox(width: AppSizes.md),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildCollapsedUserSection(
    ...
    return Center(
      child: InkWell(
        ...
        child: _buildUserAvatar(isDark, firstLetter, true),
      ),
    );
  }

  Widget _buildUserAvatar(bool isDark, String firstLetter, bool isSuperAdmin) {
    return Stack(
      children: [
        Container(
          width: 32,
