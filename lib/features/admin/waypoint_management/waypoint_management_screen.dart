...
enum MapFilter {
  ...
  String get displayName {
    switch (this) {
      case MapFilter.busStops:
        return 'Bus Stops';
      case MapFilter.busTerminals:
        return 'Bus Terminals';
      case MapFilter.currentBusLocations:
        return 'Current Bus Locations';
    }
  }
...
class _WaypointManagementScreenState extends State<WaypointManagementScreen>
  ...
  void _toggleFilter(MapFilter filter) {
    setState(() {
      if (_activeFilters.contains(filter)) {
        ...
        // Stop bus location stream if filter is removed (QUOTA OPTIMIZATION)
        if (filter == MapFilter.currentBusLocations) {
          _stopBusLocationStream();
        }
      } else {
        _activeFilters.add(filter);

        // Start bus location stream if filter is added (CONDITIONAL FETCHING)
        if (filter == MapFilter.currentBusLocations) {
          _startBusLocationStream();
        }
      }
    });
  ...
  /// Start bus location stream (only when filter is active)
  void _startBusLocationStream() {
    final authProvider = context.read<AuthProvider>();
    if (authProvider.currentUser == null) return;

    final companyId = authProvider.currentUser!.companyId;

    print('🚌 Starting bus location stream (quota-optimized)...');
    setState(() {
      _isLoadingBusLocations = true;
    });

    _busLocationSubscription?.cancel();
    _busLocationSubscription = _busLocationService
        .getBusLocationsStream(companyId)
        .listen(
          (locations) {
            if (mounted) {
              setState(() {
                _busLocations = locations;
                _isLoadingBusLocations = false;
              });
              _updateMarkers();
            }
          },
  ...
  /// Stop bus location stream (saves Firestore quota)
  void _stopBusLocationStream() {
    print('🛑 Stopping bus location stream (saving quota)...');
    _busLocationSubscription?.cancel();
  ...
  /// Format time for bus last updated
  ...
  IconData _getCategoryIcon(WaypointModel waypoint) {
  ...
  /// Get category color based on waypoint category
  Color _getCategoryColor(WaypointModel waypoint, bool isDark) {
    switch (waypoint.category) {
      case WaypointCategory.busTerminal:
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildHeaderCell('CATEGORY', isDark, flex: 2),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _getCategoryColor(waypoint, isDark).withOpacity(0.1),
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        ...
                        size: 14,
                        color: _getCategoryColor(waypoint, isDark),
                ...
                label: Row(
                  ...
                  children: [
                    Icon(
                      ...
                      color: isActive
                          ? Colors.white
                          : AppColors.getTextColor(isDark, isPrimary: false),
                    ),
  ...
  Widget _buildMapLegend(bool isDark) {
    ...
    if (_activeFilters.contains(MapFilter.busTerminals)) {
      activeItems.add(_buildLegendItem('Bus Terminal', terminalColor, isDark));
    }
    if (_activeFilters.contains(MapFilter.busStops)) {
      activeItems.add(_buildLegendItem('Bus Stop', busStopColor, isDark));
    }
    if (_activeFilters.contains(MapFilter.currentBusLocations)) {
      activeItems.add(_buildLegendItem('Active Bus', busColor, isDark));
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSizes.sm),
          ...activeItems
              .expand((item) => [item, const SizedBox(height: AppSizes.xs)])
              .toList()
